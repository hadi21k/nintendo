...
import LargeDeviceScrolledNavbar from "./LargeDeviceScrolledNavbar";
import { useBodyOverflow } from "@/hooks/useBodyOverflow";

const Navbar = () => {
  const [allCategoriesMenuIsOpen, setAllCategoriesMenuIsOpen] = useState(false);
  const [loginSignUpMenuIsOpen, setLoginSignUpMenuIsOpen] = useState(false);
  const [isInputFocused, setIsInputFocused] = useState(false);

  useBodyOverflow(!!isInputFocused);
    ...
    <nav className="h-12 relative max-lg:bg-main">
      <div className="flex h-full justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div className="border-b mb-2 border-black hover:border-main max-lg:hidden flex items-center">
            <CiSearch
              className={`text-black text-3xl ${
                isInputFocused ? "text-main" : ""
              }`}
            />
            <Input
              type="text"
              ...
              <DropdownMenuTrigger className="flex hover:text-main items-center space-x-1">
                ...
                <MdOutlineKeyboardArrowUp
                  className={`transition-transform duration-300 ${
                    allCategoriesMenuIsOpen ? "transform rotate-180" : ""
                  }`}
                />
              </DropdownMenuTrigger>
              <DropdownMenuContent>
        ...
        <div className="flex items-center gap-1">
          {!isInputFocused ? (
            ...
            <IoMdCloseCircle
              ...
              className="text-5xl px-2 max-lg:hidden cursor-pointer"
            />
          )}
        </div>
        <SearchInputMenu
      ...
      <LoginSheet
        loginSignUpMenuIsOpen={loginSignUpMenuIsOpen}
        setLoginSignUpMenuIsOpen={setLoginSignUpMenuIsOpen}
      />

      <LargeDeviceScrolledNavbar />
    </nav>
  );
};
